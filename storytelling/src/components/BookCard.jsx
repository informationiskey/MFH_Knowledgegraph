...
export default function BookCard({ node, onOpen, inView }) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      className={`group relative w-64 max-w-[80vw] rounded-xl border-t-4 ${style.border} ${
        isBlockade ? 'bg-red-950/85 ring-2 ring-red-500/60' : 'bg-slate-900/85'
      } px-5 py-4 text-left shadow-xl backdrop-blur transition-colors hover:bg-slate-800/90`}
    >
      <div className={`mb-1 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider ${style.tag}`}>
        <span className={`inline-block h-2 w-2 rounded-full ${style.dot}`} />
        {node.tag}
      </div>
      <div className={`text-lg font-bold ${isBlockade ? 'text-red-100' : 'text-slate-50'}`}>{node.title}</div>
      <div className="mt-1 text-sm text-slate-400">{node.ref}</div>
      <div className="mt-3 text-xs text-sky-300/80 opacity-0 transition-opacity group-hover:opacity-100">
        Klicken für Details →
      </div>
    </motion.button>
  )
